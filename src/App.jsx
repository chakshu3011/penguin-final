...
const getRandomItemType = () => {
  const rand = Math.random() * 105; 
  if (rand < 65) return "fish";     
  if (rand < 95) return "squid";    
  return "plastic";                 
};
...
function Squid({ position, onCollect }) {
  ...
  return (
    <Interactive onSelect={onCollect}>
      <group ref={group} position={position}>
        <primitive object={squid.scene} scale={0.5} position={[0, 0.05, 0]} />
      </group>
    </Interactive>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{ width: "100vw", height: "100dvh", overflow: "hidden", position: "relative", backgroundColor: "#0b0f19" }}>
      ...
      {/* FIX: Moved Thank You screen INSIDE the AR DOM Overlay container */}
      <div ref={setOverlayElement} style={{ position: "absolute", zIndex: 10, width: "100%", height: "100%", pointerEvents: "none", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
        {isARActive && (
          <>
            {showThanks ? (
              <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "rgba(15, 23, 42, 0.95)", zIndex: 100, color: "white", textAlign: "center", padding: "30px", pointerEvents: "auto" }}>
                ...
                <button onClick={stopGame} style={{ padding: "12px 30px", fontSize: "16px", fontWeight: "bold", borderRadius: "30px", border: "1px solid white", background: "transparent", color: "white", cursor: "pointer", pointerEvents: "auto" }}>
                  Exit Game
                </button>
              </div>
            ) : (
